# 构建初中生物MD版数据的milvus数据库（服务端模式，Attu可见）
import os
import json
import numpy as np
...
KEYWORD_COLLECTION_NAME = "biology_junior_content"  # 初中生物题型MD版集合
...
if __name__ == "__main__":
    print(f"\n--- 开始从初中生物MD版JSON文件 '{JSON_DATA_SOURCE_PATH}' 读取数据并存入 Milvus 服务端 ---")
    ...
    # --- 步骤 2: 定义 Milvus Schema（使用英文字段名，包含新增字段）---
    print("正在定义 Milvus schema（初中生物MD版题型数据）...")
    schema = MilvusClient.create_schema(auto_id=True, enable_dynamic_field=False)
    
    # 主键ID
    schema.add_field(field_name="id", datatype=DataType.INT64, is_primary=True, auto_id=True)
    
    # 初中生物数据的原有8个字段（使用英文字段名）
    ...
    schema.add_field(field_name="content", datatype=DataType.VARCHAR, max_length=65535)  # OCR文本内容，改为content字段
    # 使用ARRAY类型存储图片路径列表
    ...
    # content字段的向量表示（OCR文本的embedding）
    ...
    # --- 步骤 4: 创建索引 ---
    print(f"为集合 '{KEYWORD_COLLECTION_NAME}' 在字段 'content_vector' 上配置 AUTOINDEX。")
    ...
    # index_params.add_index(field_name="page_number", index_type="STL_SORT")
    ...
    try:
        with open(JSON_DATA_SOURCE_PATH, 'r', encoding='utf-8') as json_file:
            ...
            for item in tqdm(json_data, desc="处理初中生物MD版数据并生成嵌入"):
                ...
                content_val = item.get("content", "")  # 改为读取content字段
                ...
                try:
                    ...
                    content_embedding = embedding_list[0] # embedding_function_impl([text]) 返回 [[...]]

                    if len(content_embedding) != DIM_VALUE:
                        print(f"  错误：content '{content_val[:50]}...' 的嵌入维度 ({len(content_embedding)}) 与 Milvus 集合维度 ({DIM_VALUE}) 不匹配，已跳过。")
                        continue
                    ...
                    data_to_insert_milvus.append({
                        ...
                        "content_vector": content_embedding   # OCR文本的向量表示
                    })
                    
                except Exception as e:
                    print(f"  处理content '{content_val[:50]}...' (题型：{topic_name_val}，页面：{page_number_val}) 时出错: {e}")
    ...
    if data_to_insert_milvus:
        print(f"\n准备将 {len(data_to_insert_milvus)} 条初中生物MD版数据插入 Milvus...")
        try:
            ...
            if inserted_count > 0:
                 print(f"成功提交了 {inserted_count} 条初中生物MD版数据到 Milvus。")
    ...
    print("  content -> content (OCR文本内容，需要embedding)")
