...
process_pool = ProcessPoolExecutor(max_workers=min(multiprocessing.cpu_count() // 2 + 1, 16), mp_context=multiprocessing.get_context("spawn"))
...
get_pdf_filter = cache(lambda: PdfFilter(languages_to_keep={Language.ENGLISH, Language.CHINESE, None}, apply_download_spam_check=True, apply_form_check=True))
...
async def process_pdf(args, worker_id: int, pdf_orig_path: str):
    with tempfile.NamedTemporaryFile("wb+", suffix=".pdf") as tf:
        ...
        try:
            # 使用gather替代TaskGroup以兼容Python 3.10
            page_tasks = []
            for page_num in range(1, num_pages + 1):
                task = asyncio.create_task(process_page(args, worker_id, pdf_orig_path, tf.name, page_num))
            ...
            # 过滤掉异常结果，只保留正常结果
            page_results = [result for result in page_results if not isinstance(result, Exception)]
...
async def worker(args, work_queue: WorkQueue, semaphore, worker_id):
    while True:
        ...
        try:
            # 使用gather替代TaskGroup以兼容Python 3.10
            dolma_tasks = [asyncio.create_task(process_pdf(args, worker_id, pdf)) for pdf in work_item.work_paths]
            logger.info(f"Created all tasks for {work_item.hash}")

            # 等待所有任务完成，允许异常
            task_results = await asyncio.gather(*dolma_tasks, return_exceptions=True)
            logger.info(f"Finished processing tasks for worker on {work_item.hash}")
...
async def sglang_server_task(args, semaphore):
    ...
    mem_fraction_arg = ["--mem-fraction-static", "0.50"]
